...
class TaskGraphqlSecured extends LitElement {
    ...
    async _load() {
        this._error = null;
        try {
            const data = await Queries.securedTasks();
            this._tasks = data.securedTasks || data;
    ...
    async _add() {
        ...
        try { await Mutations.securedAddTask({title: field.value.trim()}); field.value = ''; }
        catch (e) { this._error = e.message || 'Unauthorized'; }
        this._load();
    }

    async _delete(id) {
        try { await Mutations.securedDeleteTask({id}); }
...
customElements.define('task-graphql-secured', TaskGraphqlSecured);
